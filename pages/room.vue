<template>
    ...
    <div class="flex flex-col gap-5 justify-center items-center h-screen relative">
        <div class="fixed left-[50%] translate-y-2/4">
            <ConfettiExplosion v-if="confettiVisible" :duration="5000" />
        </div>
        <UCard class="bg-gray-300 flex flex-col gap-5 justify-center items-center  w-f">
            <div class="flex gap-2 justify-between items-center  mb-3">
                <h4 class="text-gray-200 text-center"> Room Code : {{ roomStore.roomData?.room_code }}</h4>
                <UButton :icon="`${!copied ? 'i-heroicons-document-duplicate' : 'i-heroicons-clipboard'}`" size="xs"
                    :disabled="copied" color="primary" square variant="ghost" @click="copy(room_code_source)" />
            </div>
            <div
                ...
                <div class="flex w-full justify-around" v-for="(row, i) of grid" :key="i">
                    <button :disabled="!yourTurn"
                        ...
                        :class="`${cell ? 'text-[#FF9800]' : 'text-cyan-500'} ${!yourTurn ? 'cursor-not-allowed' : ''} ${yourTurn ? 'border-green-500' : 'border-red-500'}`"
            ...
            <div class=" relative rounded-lg shadow-md w-[30rem]">
                ...
                <div v-for="player in players" :key="player.user.id" :class="(player.user.id === you.id && yourTurn) ? 'ring-2 ring-gray-200 dark:ring-slate-500' : '',
                    (player.user.id !== you.id && !yourTurn) ? 'ring-2 ring-gray-200 dark:ring-slate-500' : ''"
                    ...
                    <div class="player-card__info flex items-center pointer-events-none select-none">
                        <div class="player-card__avatar w-10 h-10 m-2 rounded-md ">
                            <img class=" rounded-full"
                                :src="`https://ui-avatars.com/api/?background=9ca3af&name=` + player.user.name" alt="">
                        </div>
