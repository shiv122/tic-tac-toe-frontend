<template>
  <div class="flex justify-center items-center h-screen">
    <UCard
      class="bg-white p-4 rounded-lg shadow-md w-[30rem] divide-y-2 divide-dashed"
    >
      <template #header>
        <h4 class="text-center">Join or Create Room</h4>
      </template>
      <div class="">
...
const createRoom = async () => {
  try {
    ...
    roomStore.gameState = res.gameState;
    nextTick(async () => {
      await goto("/room");
    });
  } catch (error) {
